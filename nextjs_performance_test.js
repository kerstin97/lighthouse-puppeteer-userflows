...
for (let i = 0; i < testIterations; i++) {
  await (async () => {
    ...
    // Homepage
    await flow.navigate("http://193.170.119.145");
    // Events
    await flow.navigate("http://193.170.119.145/events");
    // Event Detail
    await flow.navigate(
      "http://193.170.119.145/events/88a85175246ccb8c58a2d6cec"
    );
    //Login Page
    await flow.navigate("http://193.170.119.145/login");

    // fill form and send
    await page.type("#username-input", "kersoleynsta");
    await page.type("#password-input", process.env.PASSWORD);

    console.log("vor klick auf speichern");
    ...
    // Create Event Page
    await flow.navigate("http://193.170.119.145/events/new");
    ...
    await page.type("input[name=info]", "pupeteer test");
    ...
    // My Events
    await flow.navigate("http://193.170.119.145/events/my-events");
    console.log("Current page:", page.url());

    await browser.close();

    writeFileSync(`report_nextjs${i}.html`, await flow.generateReport());
    writeFileSync(
      `report_nextjs${i}.json`,
...
for (let i = 0; i < testIterations; i++) {
  const inputFile = `report_nextjs${i}.json`;
  const outputFile = `report_nextjs${i}.csv`;
